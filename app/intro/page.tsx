...
type Cell = {
  letter: string | null; // huruf jawaban benar
  input: string; // huruf dari user
  isBlock: boolean; // kotak hitam
};
...
export default function Crossword() {
 ...
 const toggleMusic = () => {
  ...
  // Grid 7x8 dengan 4 kata mendatar
  ...
  const handleInput = (r: number, c: number, value: string) => {
    ...
    const correct = grid[r][c].letter?.toUpperCase() === value.slice(-1).toUpperCase();
    if (value) {
  ...
  return (
    <main className="relative min-h-screen flex flex-col items-center justify-center bg-cover bg-center text-black"
      style={{backgroundImage: "url('/teacher-vector.jpg')" }}>
          ...
          row.map((cell, c) =>
            cell.isBlock ? (
              ...
              <input
                ...
                className={`w-10 h-10 text-center font-bold uppercase rounded-md focus:outline-none ${
      ...
      <div className="mt-6 text-left w-96">
        <h2 className="font-semibold mb-2">Petunjuk Mendatar:</h2>
        <ul className="list-disc list-inside text-sm space-y-1">
          <li><b>1.</b> Bulan apa pertama kali jadian? </li>
          <li><b>2.</b> orang terganteng di hidup kamu?</li>
          <li><b>3.</b> Tempat untuk regristrasi menikah</li>
        </ul>
      </div>

            <div className="mt-6 text-left w-96">
        <h2 className="font-semibold mb-2">Petunjuk Menurun:</h2>
        <ul className="list-disc list-inside text-sm space-y-1">
      ...
      <AnimatePresence>
        {solved && (
          <motion.div
            className="absolute inset-0 bg-black/70 flex items-center justify-center text-4xl font-bold text-yellow-300"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          >
            ✨ Selesai! ✨
